...
# logging with colors
logger.add(sys.stdout, colorize=True, format="{time}</green> <level><red>{message}</red></level>")

# no handler, no formatter, no filter - one function
logger.add(sys.stderr, format="{time} <red>{level}</red> {message}", filter="my_module",  level="INFO")
